...
/>
          ...
          <div className="flex items-center gap-1 sm:gap-2 shrink-0">
            ...
            <button onClick={() => setModalOpen(true)} className={`flex items-center gap-1 sm:gap-1.5 px- sm:px-3 py-1 sm:py-1 rounded-md text-xs font-mono font-bold tracking-widest transition-all whitespace-nowrap ${user ? 'border border-lime-400/40 text-lime-400 hover:bg-lime-400/10' : 'bg-foreground text-background hover:opacity-90'}`}>
              {user ? <><User size={12} /><span className="hidden sm:inline max-w-[80px] truncate">{userProfile?.displayName ?? user.displayName ?? 'USER'}</span></> : <><LogIn size={10} />{t.login}</>}
            </button>
          </div>
        </div>
      </nav>
